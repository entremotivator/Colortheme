...
# VAPI API Key from Streamlit Secrets
try:
...
if not api_key_valid:
    st.error(f"🔑 {api_key_message}")
    st.markdown("""
    <div class="secrets-info">
        <h4>📋 How to Configure Streamlit Secrets:</h4>
        <ol>
            <li>Create a <code>.streamlit</code> folder in your project directory</li>
            <li>Create a <code>secrets.toml</code> file inside the <code>.streamlit</code> folder</li>
            <li>Add your API key: <code>VAPI_API_KEY = "your_actual_api_key_here"</code></li>
            <li>Restart your Streamlit app</li>
        </ol>
        <p><strong>Note:</strong> Never commit secrets.toml to version control!</p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with st.sidebar:
    ...
    if api_key_valid:
        ...
        masked_key = f"{'*' * (len(VAPI_API_KEY) - 4)}{VAPI_API_KEY[-4:]}" if len(VAPI_API_KEY) > 4 else "****"
    ...
    for i, (agent_name, agent_info) in enumerate(filtered_agents.items()):
        ...
        with col1 if i % 2 == 0 else col2:
            with st.container():
                ...
                # Quick call button
                if st.button(f"📞 Call {agent_name}", key=f"quick_call_{agent_name}", disabled=not api_key_valid):
...
with tab2:
    ...
    if not api_key_valid:
        st.warning("⚠️ API key not configured. Please check the Settings tab for setup instructions.")
    ...
    with col1:
        user_name = st.text_input("Your Name", 
                                  value=st.session_state.user_preferences.get('name', ''),
                                  placeholder="Enter your name")
    with col2:
        user_company = st.text_input("Company", 
                                     value=st.session_state.user_preferences.get('company', ''),
                                     placeholder="Your company")
    with col3:
        user_phone = st.text_input("Phone", 
                                   value=st.session_state.user_preferences.get('phone', ''),
    ...
    with col1:
        if st.button("▶️ Start Call", 
                     disabled=st.session_state.call_active or not api_key_valid, 
...
with tab5:
    ...
    if api_key_valid:
        st.success("✅ API Key is properly configured via Streamlit Secrets")
        masked_key = f"{'*' * (len(VAPI_API_KEY) - 4)}{VAPI_API_KEY[-4:]}" if len(VAPI_API_KEY) > 4 else "****"
        ...
        <div class="secrets-info">
            ...
            <ol>
                <li><strong>Create secrets file:</strong> Create a <code>.streamlit/secrets.toml</code> file in your project root</li>
                <li><strong>Add API key:</strong> Add the following line to secrets.toml:</li>
                <pre><code>VAPI_API_KEY = "your_actual_vapi_api_key_here"</code></pre>
                <li><strong>Security:</strong> Add <code>.streamlit/secrets.toml</code> to your <code>.gitignore</code> file</li>
                <li><strong>Restart:</strong> Restart your Streamlit application</li>
            </ol>
            <p><strong>⚠️ Important:</strong> Never commit secrets.toml to version control!</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Secrets Configuration Help
    st.subheader("📖 Secrets Configuration Guide")
    with st.expander("📚 Detailed Setup Instructions"):
        ...
        **Step 3: Add your API key to secrets.toml**
        ```toml
    ...
    with st.expander("💡 Using Environment Variables"):
        st.markdown("""
...
<div style="text-align: center; color: #666; padding: 2rem;">
    ...
    <p>Built with ❤️ using Streamlit • Secured with Streamlit Secrets</p>
</div>
""", unsafe_allow_html=True)
